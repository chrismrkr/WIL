 ...
 **@MappedSuperClass와 @Embeddable의 차이점은?**
 ...
 ### 3.6 복합 키 
복합 키란 2개 이상의 속성으로 이루어진 기본 키를 의미한다. @IdClass 또는 @Embeddable로 구현할 수 있다.

복합 키는 아래 3가지 조건이 필요하다.
...
복합 키 생성 시 어떤 방식을 사용해도 문제 없다. 그러나, 조인 테이블을 생성하는 경우에는 @EmbeddedId, @Embeddable을 사용할 수 없다.
