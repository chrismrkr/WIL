...
- 디스플레이 장치에 의존적이지 않음
- 라이센스 문제로 인하여 XFree86을 사용하지 않고 X.org 사용
#### 2.3.2 X 윈도우 구조
- X 서버, X 클라이언트
...
- X 윈도우 프로젝트
#### 2.3.4 X 윈도우 계층
- 디스플레이 매니저 : 로그인을 위한 그래픽 유저 인터페이스 제공(ex. GNOME, KDE)
- X 세션 : X 서버에 X 클라이언트가 연결되면 맺어짐
- 윈도우 매니저
#### 2.3.5 데스크톱환경 구성 사례
#### 2.3.6 X 윈도우 실행
- CentOS6
...
```
- xhost + : X 서버에서 특정 또는 모든 클라이언트 접속 허용
- xhost - : X 서버에서 특정 또는 모든 클라이언트 접속 차단
- xauth : $HOME/.Xauthority 파일 안의 MIT-MAGIC-COOKIEs를 이용하여 X 서버에 대한 접근 제어
- DISPLAY 환경변수 : X 클라이언트 프로그램의 화면 출력을 특정 ip로 보냄
  - ex. export DISPLAY="192.168.100.41:0.1 : 화면 출력을 해당 ip의 두번째 모니터로 보냄
  - 특정 원격지에 화면 출력을 보내기 위해 사용함
#### 2.3.7 X 윈도우 관련 프로그램
- KWin : X 윈도우 시스템 창 관리자
- Metacity : GNOME2 데스크톱 환경을 위한 기본 창 관리자
- Mutter : GNOME3 데스크톱 환경을 위한 기본 창 관리자 
...
- 포어그라운드-백그라운드 전환
  - 포어그라운드 -> 백그라운드 전환 : ctrl+z 또는 ```bg```명령어로 백그라운드 실행 가능(```jobs```명령어로 현재 백그라운드 실행 프로세스 확인 가능)
  - 백그라운드 -> 포어그라운드 전환 : ```fg %[작업번호]```
    - PID와 작업번호를 혼동해서는 안됨
##### 2.5.2.3 프로세스 중지
- ctrl+c, kill
...
- ntsysv : 텍스트 환경에서 커서를 이용하여 부팅 중 자동으로 실행되는 서비스를 설정하는 유틸리티
...
- 프로토콜 제정 위원회
  ...
  - 전자산업협회(EIA) : 물리적 연결 인터페이스와 전자신호 규격 규정(미국 에너지부 통계청)
  - 한국정보통신기술협회(TTA)
  - ICANN : 도메인 이름, IP 주소, 프로토콜 범주와 포트번호 감독 및 도메인 관련 분쟁 조율
##### 3.1.4.2 TCP/IP
...
- 11111111.11111111.11111111.xx000000을 서브넷 마스크로 하여 4개의 서브넷을 생성한다고 가정
  ...
  - 서브넷당 ip 3개는 미리 할당됨(첫번째 주소: 네트워크 주소, 마지막 주소: 브로드캐스팅용 주소, Gateway용 주소)
##### 3.1.4.5 도메인
- ex. www.google.com
  - 최상위 도메인 : .com
  - 도메인 이름 : google
...
- 네트워크 관련 정보 출력하는 명령어
  ...
  - TIME_WAIT : ACK를 받고 종료까지 기다리는 상태
##### 3.2.3.5 traceroute
...
- 찾지 못하는 경우 **/etc/resolv.conf**에서 nameserver를 확인해야함
