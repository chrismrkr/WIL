...
## 17. 자바스크립트 엔진과 브라우저 엔진의 동작방식

자바스크립트 엔진은 싱글 스레드로 동작하는데, 비동기적인 작업을 동시에 처리할 수 있다는 점이 다소 아이러니했다.

결론은 자바스크립트 엔진은 싱글 스레드로 동작하나 브라우저 엔진(V8)이 멀티 스레드로 동작하기 때문에 가능한 것이었다.

동작 방식은 아래와 같다.

+ 1. 클라이언트의 event-driven 요청 또는 자바스크립트가 제공하는 비동기적 요청 발생
+ 2. 브라우저 엔진에서 멀티 스레드 방식으로 이를 받음.
+ 3. 요청의 실제적인 비즈니스 로직을 callback으로 taskQueue와 microtaskQueue에 전달.
+ 4. 클라이언트에 응답 제공

여기까지 브라우저 엔진이 작업한다. 그 이후로는 자바스크립트에서 담당한다.

+ 1. Event Loop에서 callStack이 비어 있고, taskQueue 또는 microtaskQueue가 비어있지 않은지 non-blocking하게 수시로 확인
+ 2. 위 조건을 만족하면 Queue에 있는 것을 callStack으로 전달
+ 3. 작업이 완료되면 callstack pop

위 과정을 통해 자바스크립트는 싱글 스레드임에도 불구하고 비동기적인 동시처리가 가능하다.

위를 응용하면, 자바스크립트 기반 서버 프레임워크 Nest.js는 요청량이 많고 리소스 점유시간이 적은 서비스에 유용하게 사용될 수 있을 것으로 보인다.
