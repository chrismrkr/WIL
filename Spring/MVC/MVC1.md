...
+ 스프링 MVC에서 사용되는 Annotation의 역할과 동작방식에 대해서 설명할 수 있다.
...
스프링 MVC를 학습하기 전에 서블릿에 대해서 알아보도록 한다. 서블릿은 서버에서 Request-Response를 처리하기 위한 프로그램이다.

HTTP 요청-응답을 처리하는 서버 프로그램을 HttpServlet이라고 한다.
...
그러나, HTTP 메세지를 직접적으로 받는 곳은 프론트 컨트롤러이다. 프론트 컨트롤러를 제외한 컨트롤러들에서 HttpServlet을 항상 사용할 이유는 없다.

여기서 등장한 개념이 Model이다. 흐름은 아래와 같다.

1. 프론트 컨트롤러에서 HTTP 요청을 받고, 요청에 맞는 컨트롤러를 호출한다.

2. URI와 HTTP 헤더 정보, 쿼리파라미터, 메세지바디 내용을 저장하는 Map<String, String>을 생성한다.
