...
그러므로, Isolation level을 조절해서 위 문제를 막을 수 있다. 그러나, 위 문제를 막으려고 할수록 Dead Lock 발생 가능성이 높아진다는 trade-off가 있다.
...
**commit되지 않은 데이터는 undo 로그에서 값을 가져온다는 정책이다.** 그러므로, 위의 dirty read는 예제는 막을 수 있다.

read(A)를 할 때, A는 commit되지 않은 데이터이므로 undo 로그에서 값을 가져온다.

그러나, non-repeatable read 문제가 발생한다. commit된 데이터는 undo로그가 아닌 테이블에서 읽으므로 문제가 발생한다.
...
동일한 트랜잭션에서는 Read에 대해서는 부정합이 발생하지 않는다.

commit 여부와 관계 없이 undo 로그 레코드에 백업된 데이터를 보여준다. 그러나, phantom read가 발생할 수 있다.

T1: -------Select for update 시작----------------------Select for update 종료

T2: --------------------------------insert----commit---------------------------

#### 3.3.4 Serializable

Read, Write 작업에 대해서 모두 Exclusive lock을 갖는다.

그러므로, 위의 모든 문제는 발생하지 않지만 Dead Lock이 발생할 가능성이 높아진다.

### 3.4 낙관적 Lock 기법

트랜잭션 충돌은 일어나지 않을 것이라고 가정한다.

commit 이전에 레코드의 timestamp 또는 version을 이용해 conflict가 있는지 없는지를 확인한 후 commit 하는 기법이다.
...
+ <T, start>
+ <T, command, oldVal, newVal>
+ <T, commit>
+ <T, abort>

+ Atomicity를 위해서 변경된 모든 사항을 redo 로그에 저장한다.
+ Durability를 위해 commit 전에 모든 내용을 undo 로그에 저장한다. 

### 4.2 데이터 복구 연산

+ redo : 실행된 모든 변경 연산을 재실행해서 장애 발생 직전으로 되돌린다.
+ undo : 지금까지 실행된 모든 연산을 취소하여 원래 상태로 복구한다.


### 4.3 로그 회복 기법

+ 즉시 갱신 회복 기법 : 변경 결과는 즉시 DB에 반영되고, 장애 시 commit 완료된 것은 redo 연산을 하고, commit되지 않은 것은 undo 연산을 한다.
+ 지연 갱신 회복 기법 : 로그파일에 저장하고 commit 시 DB에 반영한다. 장애 시 commit 완료된 것은 redo 연산을 하고, commit되지 않은 것은 버리면 된다.

### 4.4 검사 시점 회복 기법

로그를 기록하되 일정시간 간격으로 checkpoint를 설정한다. 그리고, 장애 발생시 체크 포인트 이후 로그 기반으로만 복구한다.
...
+ 제 1 정규형 : 모든 속성은 반드시 1개의 값을 갖는다.
+ 제 2 정규형 : 모든 속성은 기본키의 일부가 아닌 전체에 종속된다.
+ 제 3 정규형 : 속성간 이행적 종속성이 없어야 한다. 

### 5.2 이상현상

+ 삽입 이상 : 특정 값을 저장해야 하는데, 반드시 다른 값도 함께 저장하는 경우
+ 갱신 이상 : 특정 값 1개를 변경하고 싶을 때, 다른 레코드 값도 함께 업데이트 해야하는 경우
+ 삭제 이상 : 특정 값을 삭제하면, 그 값이 테이블에서 아예 소멸되는 현상

그러므로, 이상현상을 막기 위해서는 엔티티(테이블) 역할에 맞게 잘 정의해야 한다.
