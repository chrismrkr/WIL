...
1. 프로세스가 필요한 메모리마다 가상 메모리 주소가 존재한다. 
2. 실행하고자 하는 프로그램이 필요한 메모리의 크기를 페이지 단위(ex. 4KB)로 나눈다.
3. 접근하고자 하는 페이지(메모리의 부분)가 PageTable에 존재한다면 그것을 실행한다.
4. 만약 존재하지 않는다면, Page를 할당한다. 만약, 다른 것이 존재한다면 대체한다.(replacement)
