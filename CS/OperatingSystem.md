...
#### 1.3.6 교착상태(dead lock)

아래 4가지를 만족하면 교착상태가 발생한다.

+ 상호 배제성 : 특정 자원을 점유하면, 다른 스레드(프로세스)가 그 자원에 접근 불가능
+ 점유 및 대기 : 한 자원을 점유한 상태에서, 다른 자원을 점유하는 것을 기다린다.
+ 비선점 : 사용할 자원을 외부에서 끊을 수 없다.
+ 선형대기 : wait 그래프가 순환 상태이다.

뮤텍스, 세마포어, 모니터는 상호 배제성 문제만 해결한다.

교착상태를 피하기 위해서는 4개 중 하나가 만족하지 않도록 해야 한다. 그러나, 자원 사용의 효율성을 떨어뜨릴 수 있다.
